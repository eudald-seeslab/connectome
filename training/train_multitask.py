...
from configs.config_multitasking_dirs import data_dirs_config
...
def get_multitask_batch_data(
    ...
    if i % config_.wandb_images_every == 0:
        ...
        tracker.log_image(p, basename(batch_files[0]), title, task=task)
        ...
        for ep in range(config.num_epochs):
            ...
            for i in tqdm(range(iterations)):
                ...
                for idx, (out, loss, inp, lbl) in enumerate(
                    ...
                    tracker.log_metrics(
                        ep,
                        running_losses[idx] / totals[idx],
                        total_correct[idx] / totals[idx],
                        task=idx + 1,
                    )
