...
nodes["visual"] = nodes["additional_annotations"].apply(lambda x: True if x == "visual" else False)
...
nodes = nodes.merge(adj_matrix.index.to_frame(), left_on="skid", right_index=True, how="right")
nodes = nodes.reset_index(drop=True)
nodes = nodes.drop(columns=[0, "additional_annotations", "level_7_cluster", "hemisphere"])
...
# Put adj_matrix to a new dataframe in longitudinal form and set 1 if there is a connection between two neurons and 0 otherwise
