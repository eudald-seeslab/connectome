"""Synapse length distribution plots."""
...
def plot_synapse_length_distributions(
    neuron_coords, conns_dict, use_density=True, num_confidence_interval_se=1
):
    """
    ...
    -------
    tuple
        (fig1, fig2) - Two figure objects for histogram and synapse strength vs distance
    """
    ...
    dists = {
        name: compute_individual_synapse_lengths(df, neuron_coords)
        for name, df in conns_dict.items()
    }
    weights = {name: df["syn_count"].to_numpy() for name, df in conns_dict.items()}
    ...
    for name in titles:
        hist, _ = np.histogram(
            dists[name], bins=bins, weights=weights[name], density=use_density
        )
    ...
    fig1, axs1 = plt.subplots(
        n_plots, 1, figsize=(12, 2.5 * n_plots), sharex=True, constrained_layout=True
    )
    ...
    for ax, title in zip(axs1, titles):
        ...
        ax.hist(
            ...
            weights=w,
            density=use_density,
            color=get_randomization_colors(title),
            alpha=0.7,
        )

        mean_nm = np.average(L, weights=w)
        ax.axvline(mean_nm, ls="--", c="k", lw=1)
        ax.text(mean_nm * 1.05, 0.7 * max_val, f"Mean: {mean_nm / 1e3:,.2f} µm")
    ...
    bin_edges = np.linspace(0, max_len, 20)
...
def plot_synapse_counts_histogram(conns_dict, bins=30, figsize=None, log_scale=False):
    ...
    figsize : tuple, default=None
        Figure size (width, height). Auto-scales if None.
    ...
    fig, axs = plt.subplots(
        n_plots, 1, figsize=figsize, sharex=True, constrained_layout=True
    )
