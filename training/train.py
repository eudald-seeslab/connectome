...
from connectome.data_helpers import get_logger
from trainyourfly.connectome_models.graph_models_helpers import EarlyStopping, TrainingError
from configs import config
from trainyourfly.plots.plots import guess_your_plots, plot_results
...
def main(tracker, sweep_config=None):
    ...
    model_name = f"m_{start_datetime}_{tracker.run_id}{dchar}.pth"
        ...
        for ep in range(u_config.num_epochs):
            ...
            for i in tqdm(range(iterations)):
                ...
                if i % u_config.wandb_images_every == 0:
                    ...
                    tracker.log_image(p, basename(batch_files[0]), title)
                ...
                tracker.log_metrics(ep, running_loss / total, total_correct / total)
                if i == 0:
    ...
    tracker.log_validation(
        running_loss / total, total_correct / total, test_results, final_plots
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.error(error)
        tracker.send_alert(f"Error during training: {error}")
